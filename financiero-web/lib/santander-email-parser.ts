...
function esTextoPromocionalSantander(texto: string) {
  return /(&[a-z]+;|tu\s+navegador|si\s+est(?:ás|&aacute;s)\s+pensando|junio\s+20\d{2}|\b(msi|meses\s+sin\s+intereses|difiere|diferir|haz\s+compras|compras\s+mayores|promoci[oó]n|oferta|beneficio|bonificaci[oó]n|descuento|participa|contrata|terminales?\s+clip|aprovecha|l[ií]nea\s+de\s+cr[eé]dito|cr[eé]dito\s+preaprobado|incrementa\s+tu\s+l[ií]nea)\b)/i.test(texto) &&
