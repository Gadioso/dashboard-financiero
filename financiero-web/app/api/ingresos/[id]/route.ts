...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    const deleteQuery = supabase
      ...
      .delete()
      .eq('id', id)
      .select('id, fecha, bank_transaction_raw_id');
    ...
    if (data.bank_transaction_raw_id) {
      ...
      if (bankTransactionError) {
        return NextResponse.json({ success: false, error: 'El ingreso se eliminó, pero no pude evitar que el banco lo importe nuevamente.' }, { status: 500 });
      }
    }
